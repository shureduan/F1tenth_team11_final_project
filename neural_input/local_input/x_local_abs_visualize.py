...
XLOCAL_CSV = os.path.join(RACELINE_DIR, "x_local_features.csv")
...
def percentile_sqrt_boost(values):
    """
    Kept for compatibility, but not used by Fig.1.
    """
...
def build_dense_adaptive_arrows(x, y, delta_psi):
    ...
    while i < n:
        ...
        # overall about 2x sparser than before
        if ai < 0.12:
            X.append(x[i]); Y.append(y[i]); U.append(ux[i]); V.append(uy[i]); i += 6
        elif ai < 0.22:
            X.append(x[i]); Y.append(y[i]); U.append(ux[i]); V.append(uy[i]); i += 4
        elif ai < 0.38:
...
def main():
    ...
    # Fig.1 curvature visualization: physical |kappa|
    abs_kappa = np.abs(kappa)
    abs_kappa_vis = circular_gaussian_smooth(abs_kappa, sigma=1.6)

    # dead-zone for straight segments: tiny curvature should look blue
    ...
    diag = np.sqrt((np.max(all_xy[:, 0]) - np.min(all_xy[:, 0]))**2 +
                   (np.max(all_xy[:, 1]) - np.min(all_xy[:, 1]))**2)
