...
All features (d_left, d_right, delta_kappa, delta_psi, boundary indices)
are recomputed here; no dependency on x_local_features.csv.
"""
...
OUT_FIG1 = os.path.join(OUT_DIR, "segment_abs_curvature_track_heatmap.png")
OUT_FIG2 = os.path.join(OUT_DIR, "segment_inner_space_dkappa_track_panels.png")
...
def build_dense_adaptive_arrows(x, y, delta_psi):
    ...
    while i < n:
        ...
        ai = a[i]
        if ai < 0.12:
            X.append(x[i]); Y.append(y[i]); U.append(ux[i]); V.append(uy[i]); i += 48
        elif ai < 0.22:
            X.append(x[i]); Y.append(y[i]); U.append(ux[i]); V.append(uy[i]); i += 32
        elif ai < 0.38:
            X.append(x[i]); Y.append(y[i]); U.append(ux[i]); V.append(uy[i]); i += 16
        elif ai < 0.60:
            X.append(x[i]); Y.append(y[i]); U.append(ux[i]); V.append(uy[i])
            xm = 0.5*(x[i]+x[j]); ym = 0.5*(y[i]+y[j])
            um = x[j]-x[i]; vm = y[j]-y[i]; nn = np.sqrt(um**2+vm**2)+1e-12
            X.append(xm); Y.append(ym); U.append(um/nn); V.append(vm/nn); i += 16
        elif ai < 0.82:
            X.append(x[i]); Y.append(y[i]); U.append(ux[i]); V.append(uy[i])
            x1=x[i]+(x[j]-x[i])/3; y1=y[i]+(y[j]-y[i])/3
            x2=x[i]+2*(x[j]-x[i])/3; y2=y[i]+2*(y[j]-y[i])/3
            um=x[j]-x[i]; vm=y[j]-y[i]; nn=np.sqrt(um**2+vm**2)+1e-12; um/=nn; vm/=nn
            X.extend([x1,x2]); Y.extend([y1,y2]); U.extend([um,um]); V.extend([vm,vm]); i += 16
        else:
            X.append(x[i]); Y.append(y[i]); U.append(ux[i]); V.append(uy[i])
            x1=x[i]+.25*(x[j]-x[i]); y1=y[i]+.25*(y[j]-y[i])
            x2=x[i]+.50*(x[j]-x[i]); y2=y[i]+.50*(y[j]-y[i])
            x3=x[i]+.75*(x[j]-x[i]); y3=y[i]+.75*(y[j]-y[i])
            um=x[j]-x[i]; vm=y[j]-y[i]; nn=np.sqrt(um**2+vm**2)+1e-12; um/=nn; vm/=nn
            X.extend([x1,x2,x3]); Y.extend([y1,y2,y3])
            U.extend([um,um,um]); V.extend([vm,vm,vm]); i += 16
...
def main():
    ...
    x     = rl["x_m"].to_numpy(float)
    y     = rl["y_m"].to_numpy(float)
    kappa = rl["kappa_radpm"].to_numpy(float)

    left_xy  = lbd[["x", "y"]].to_numpy(float)
    right_xy = rbd[["x", "y"]].to_numpy(float)

    print(f"Raceline : {len(x)} pts")
    print(f"Left bd  : {len(left_xy)} pts")
    print(f"Right bd : {len(right_xy)} pts")

    # Compute all features from the current data
    (d_left, d_right,
     left_idx, right_idx,
     delta_kappa, delta_psi) = compute_features(x, y, kappa, left_xy, right_xy)

    inner_space = compute_inner_space(kappa, d_left, d_right)
    all_xy = np.vstack([left_xy, right_xy])

    # Smooth |kappa| for heatmap
    abs_kappa_vis = circular_gaussian_smooth(np.abs(kappa), sigma=1.6)
