...
LOSS_PLOT_PATH = OUT_DIR / "train_val_loss_u.png"
LOSS_CSV_PATH = OUT_DIR / "train_val_loss_history_u.csv"
...
def make_train_val_masks(n_segments: int, val_every: int = 5) -> Tuple[np.ndarray, np.ndarray]:
    """
    Deterministic split on one loop track: every val_every-th segment goes to validation.
    This keeps train/val distributed around the whole track.
    """
    idx = np.arange(n_segments)
    val_mask = (idx % val_every) == 0
    train_mask = ~val_mask
    if not np.any(val_mask) or not np.any(train_mask):
        raise ValueError("Invalid train/val split. Please check n_segments and val_every.")
...
def plot_loss_curves(history_df: pd.DataFrame, out_path: Path) -> None:
    plt.figure(figsize=(8, 5))
    plt.plot(history_df["epoch"], history_df["train_total_loss"], label="Train total loss")
    plt.plot(history_df["epoch"], history_df["val_total_loss"], label="Val total loss")
    plt.plot(history_df["epoch"], history_df["train_strategy_loss"], label="Train strategy loss", alpha=0.8)
    plt.plot(history_df["epoch"], history_df["val_strategy_loss"], label="Val strategy loss", alpha=0.8)
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
...
def main():
    ...
    train_mask_np, val_mask_np = make_train_val_masks(n_segments, val_every=5)
    train_mask_t = torch.from_numpy(train_mask_np).to(DEVICE)
    val_mask_t = torch.from_numpy(val_mask_np).to(DEVICE)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for x_seq_b, x_prior_b, y_b, d_prior_b in loader:
            ...
            train_strategy = masked_mse(d_pred, y_b, train_mask_t)
            train_prior = masked_mse(d_pred, d_prior_b, train_mask_t)
            train_smooth = smoothness_loss(d_pred)
            train_total = (
                LAMBDA_STRATEGY * train_strategy
                + LAMBDA_SMOOTH * train_smooth
                + LAMBDA_PRIOR * train_prior
            )
        ...
        model.eval()
        with torch.no_grad():
            d_pred_eval = model(x_seq_b, x_prior_b)

            train_strategy_eval = masked_mse(d_pred_eval, y_b, train_mask_t)
            train_prior_eval = masked_mse(d_pred_eval, d_prior_b, train_mask_t)
            train_smooth_eval = smoothness_loss(d_pred_eval)
            train_total_eval = (
                LAMBDA_STRATEGY * train_strategy_eval
                + LAMBDA_SMOOTH * train_smooth_eval
                + LAMBDA_PRIOR * train_prior_eval
            )

            val_strategy_eval = masked_mse(d_pred_eval, y_b, val_mask_t)
            val_prior_eval = masked_mse(d_pred_eval, d_prior_b, val_mask_t)
            val_total_eval = (
                LAMBDA_STRATEGY * val_strategy_eval
                + LAMBDA_PRIOR * val_prior_eval
            )

        history.append({
            "epoch": epoch,
            "train_total_loss": float(train_total_eval.item()),
            "train_strategy_loss": float(train_strategy_eval.item()),
            "train_smooth_loss": float(train_smooth_eval.item()),
            "train_prior_loss": float(train_prior_eval.item()),
            "val_total_loss": float(val_total_eval.item()),
            "val_strategy_loss": float(val_strategy_eval.item()),
            "val_prior_loss": float(val_prior_eval.item()),
        })

        if epoch == 1 or epoch % 20 == 0 or epoch == EPOCHS:
            print(
                f"Epoch {epoch:03d} | "
                f"Train_total={train_total_eval.item():.6f} | "
                f"Train_strategy={train_strategy_eval.item():.6f} | "
                f"Train_smooth={train_smooth_eval.item():.6f} | "
                f"Train_prior={train_prior_eval.item():.6f} | "
                f"Val_total={val_total_eval.item():.6f} | "
                f"Val_strategy={val_strategy_eval.item():.6f} | "
                f"Val_prior={val_prior_eval.item():.6f}"
            )
